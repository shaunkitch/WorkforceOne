...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: createError } = await supabaseAdmin.rpc('exec_sql', {
      sql: `
        CREATE TABLE IF NOT EXISTS backup_requests (
          ...
          distance_to_checkpoint INTEGER, -- distance in meters
          notes TEXT,
    ...
    if (createError) {
      console.error('Error creating backup_requests table:', createError)
      return NextResponse.json({
        error: 'Failed to create backup_requests table',
    ...
    return NextResponse.json({
      success: true,
      message: 'backup_requests table created successfully'
    })
