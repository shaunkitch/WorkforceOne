-- Fixed Patrol Performance System Schema
-- Resolves PostgreSQL generated column dependency issue

-- Drop existing tables if they exist (in case of retry)
DROP TABLE IF EXISTS patrol_timing_analysis CASCADE;
DROP TABLE IF EXISTS patrol_performance_metrics CASCADE;
DROP TABLE IF EXISTS patrol_requirements CASCADE;
DROP VIEW IF EXISTS patrol_performance_summary CASCADE;

-- Table for configurable patrol requirements per route
CREATE TABLE patrol_requirements (
...
-- Enhanced patrol performance metrics (FIXED: no generated column dependencies)
CREATE TABLE patrol_performance_metrics (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  ...
  patrol_distribution_score numeric DEFAULT 0, -- 0-100 score (manually calculated)
  ...
  proactive_score numeric DEFAULT 0, -- Bonus points (manually calculated)
  
  -- Overall performance score (calculated from BASE columns only)
  overall_performance_score numeric GENERATED ALWAYS AS (
    CASE WHEN required_patrols > 0 THEN
      ROUND(
        -- Compliance: completed/required * 100 * 30%
        ((completed_patrols::numeric / required_patrols::numeric) * 100 * 0.3) +
        -- Checkpoint quality: completed/required * 100 * 25%
        (CASE WHEN total_checkpoints_required > 0 
         THEN (total_checkpoints_completed::numeric / total_checkpoints_required::numeric) * 100 * 0.25
         ELSE 0 END) +
        -- Photo compliance: photos_taken/required * 100 * 15%
        (CASE WHEN photos_required > 0 
         THEN (photos_taken::numeric / photos_required::numeric) * 100 * 0.15
         ELSE 0 END) +
        -- Distribution score: 20%
        (patrol_distribution_score * 0.2) +
        -- Proactive score: 10%
        (proactive_score * 0.1),
        2
      )
...
CREATE TABLE patrol_timing_analysis (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_patrol_requirements_route_duration 
ON patrol_requirements(route_id, shift_duration_hours);

CREATE INDEX idx_patrol_performance_guard_date 
ON patrol_performance_metrics(guard_id, shift_date);

CREATE INDEX idx_patrol_performance_org_date 
ON patrol_performance_metrics(organization_id, shift_date);

CREATE INDEX idx_patrol_timing_performance 
